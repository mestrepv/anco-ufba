...
def test_painel_anco_sem_triagem_mostra_corpus(client, proj_anco):
    ana = _analista("ana3")
    client.force_login(ana)
...
def test_autotriar_redireciona_no_anco(client, proj_anco):
    """Autotriagem descontinuada: no ANCO a rota só redireciona ao corpus."""
    ...
    client.force_login(cur)
    resp = client.get(reverse("triagem_autotriar", args=[proj_anco.slug]))
    assert resp.status_code == 302
